...
(function(window, angular, undefined) {
  ...
  angular.module('ngIdle.idle', [ 'ngIdle.keepalive', 'ngIdle.localStorage' ]).provider(
    'Idle',
    function() {
      var options = {
      ...
      /**
       * Sets the number of seconds a user can be idle before they are
        ...
        function($interval, $log, $rootScope, $document, Keepalive, LocalStorage, $window) {
          ...
          var svc = {
            ...
            watch : function(noExpiryUpdate) {
              ...
              // calculate the absolute expiry date, as added insurance against
              // a browser sleeping or paused in the background
              ...
              if (state.idling)
                ...
                startKeepalive(); // if about to run, start keep alive
            ...
            interrupt : function(noExpiryUpdate) {
              ...
              if (options.autoResume === 'idle' || (options.autoResume === 'notIdle' && !state.idling))
  ...
  angular.module('ngIdle.title', []).factory('Title', [ '$document', '$interpolate', function($document, $interpolate) {
    ...
    return {
      ...
      store : function(overwrite) {
        if (overwrite || !state.original)
          state.original = this.value();
      },
      value : function(val) {
        if (angular.isUndefined(val))
  ...
  angular.module('ngIdle.localStorage', []).service('IdleLocalStorage', [ '$window', function($window) {
