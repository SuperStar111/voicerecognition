...
(function($) {
	$.fn.scriptcam=function(options) {
		...
		//	$.scriptcam.SC_promptWillShow=data.promptWillShow;
			...
			if (fileExists(decodeURIComponent(data.path)+'js/webc/scriptcam.swf')) {
				swfobject.embedSWF(decodeURIComponent(data.path)+'js/webc/scriptcam.swf', opts.id, newWidth, newHeight, '11.6', false, opts, params);
			}
			else {
				alert(decodeURIComponent(data.path)+'js/webc/scriptcam.swf not found, please check the path parameter');
			}
		});
	};
