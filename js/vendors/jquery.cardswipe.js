/*! jQuery.CardSwipe Magnetic Stripe Card Reader - v1.0.1 - 2015-09-30
...
(function(factory) {
  ...
  var plugin = function(method) {
    // Method calling logic. If named method exists, execute it with passed
    // arguments
    if (methods[method]) {
  ...
  var builtinParsers = {
    ...
    visa : function(rawData) {
      // Visa issuer number begins with 4 and may vary from 13 to 19 total
      // digits. 16 digits is most common.
    ...
    // Visa card parser.
    giftcard : function(rawData) {
      // Visa issuer number begins with 4 and may vary from 13 to 19 total
      // digits. 16 digits is most common.
      var pattern = new RegExp("^%B([0-9]{6})([0-9]{6})([0-9]{7})[\^]{2}");
    ...
    discover : function(rawData) {
      // American Express starts with 34 or 37, and is 15 digits long.
      var pattern = new RegExp("^%B(6011[0-9]{12})\\^([A-Z ]+)/([A-Z ]+)\\^([0-9]{2})([0-9]{2})");
  ...
  var listener = function(e) {
    ...
    switch (state()) {
      ...
      // PENDING: Look for A-Z, then jump to READING. Otherwise, pass the
      // keypress through, reset and jump to IDLE.
      case states.PENDING:
      ...
      // READING: Copy characters to buffer until newline, then process the
      // scanned characters
      case states.READING:
        processCode(e.which);
        startTimer();
  ...
  var processScan = function() {
    ...
    if (settings.rawDataCallback) {
      settings.rawDataCallback.call(this, rawData);
    }
    ...
    if (result) {
      ...
      // All parsers failed.
      if (settings.failure) {
        settings.failure.call(this, rawData);
      }
  ...
  var luhnChecksum = function(digits) {
    ...
    // Proceed right to left. Even and odd digit positions are handled
    // differently.
    var n = digits.length;
    var odd = true;
    while (n--) {
  ...
  // Attach internal functions to plugin for easier testing. These aren't
  // intended for
